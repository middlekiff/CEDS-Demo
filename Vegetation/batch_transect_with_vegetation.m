...
transect_coords = struct( ...
    "A", struct("x1", 1350, "x2", 950, "y1", 400, "y2", 400, "vegstart",80, "vegend",90), ...
    "B", struct("x1", 1300, "x2", 900, "y1", 700, "y2", 700, "vegstart",80, "vegend",90), ...
    "C", struct("x1", 1250, "x2", 850, "y1", 1000, "y2", 1000, "vegstart",80, "vegend",90));

% vegstart and vegend represent the starting and ending points of the
% vegetation along the transect, expressed as a percentage, with 0 being
% where the waves start generating, and 100 being on the opposite side.

% This script will create a vegmap file that tells XBeach where vegetation
% is located, but you will need to open veggiefile.txt and replace the
% contents with the name of the file that has your vegetation
% characteristics, such as weeds.txt. Then XBeach will reference that file
% when it simulates the vegetation.
...
for k = 1:length(transects)
    ...
    %% Vegetation Setup
    % Loading the transect bed
    bed     = load([outdir '\bed.dep']);
    x = 0:length(bed)-1;

    vegStart = coords.vegstart;
    vegEnd = coords.vegend;
    ...
    legend('veg','bed')
    title(['Transect ' char(name) ' Vegetation']);
    xlabel('% Distance');
    ylabel('Elevation (m)');
    saveas(gcf, fullfile(outdir, ['transect_' char(name) '_vegetation.png']));


    %% 2. Write the veg to a vegmap file
    fileID = fopen([outdir '\vegmap.txt'],'w');
    for ii=1:length(veg)
    ...
    copyfile(fullfile(pwd, 'veggiefile.txt'), outdir);
    copyfile(fullfile(pwd, 'weed.txt'), outdir); % replace this with name of your vegetation document.
